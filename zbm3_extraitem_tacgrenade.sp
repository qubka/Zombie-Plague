 ...
 * Record plugin info.
 **/
public Plugin TacGrenade =
{
	name        	= "[ZP] ExtraItem: Tactical Awareness Grenade",
 ...
 **/
#define EXTRA_ITEM_NAME				"@Tactical Awareness Grenade" // If string has @, phrase will be taken from translation file		
...
public Action ZP_OnClientBuyExtraItem(int clientIndex, int extraitemIndex)
{
	...
	if(extraitemIndex == iItem)
	{
		...
		if(IsPlayerHasWeapon(clientIndex, "weapon_tagrenade") || ZP_IsPlayerZombie(clientIndex) || ZP_IsPlayerSurvivor(clientIndex))
		{
			return Plugin_Handled;
		}

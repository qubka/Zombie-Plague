...
{
    // Items
    gItem = ZP_GetExtraItemNameID("inc grenade");
    if(gItem == -1) SetFailState("[ZP] Custom extraitem ID from name : \"inc grenade\" wasn't find");
    
    // Weapons
    gWeapon = ZP_GetWeaponNameID("inc grenade");
    gDublicat = ZP_GetWeaponNameID("molotov"); /// Bugfix
    if(gWeapon == -1 || gDublicat == -1) SetFailState("[ZP] Custom weapon ID from name : \"inc grenade\" wasn't find");
}
...
public void ZP_OnClientBuyExtraItem(int clientIndex, int extraitemIndex)
{
    ...
    if(extraitemIndex == gItem)
    {
        ...
        ZP_GiveClientWeapon(clientIndex, "inc grenade");
    }
}

...
void Weapon_OnSecondaryAttack(int client, int weapon, int iClip, int iAmmo, int iStateMode, float flCurrentTime)
{
	...
	if (!iStateMode)
	{
		...
		ZP_SetPlayerAnimation(client, PLAYERANIMEVENT_SILENCER_ATTACH );
	...
	else
	{
		...
		ZP_SetPlayerAnimation(client, PLAYERANIMEVENT_SILENCER_DETACH);
...
void Weapon_OnCreateExplosion(int client, int weapon)
{
	...
	if (TR_DidHit()) 
	{
		...
		ZP_EmitAmbientSound(gSound, 2, vEndPosition, SOUND_FROM_WORLD);
	}
}

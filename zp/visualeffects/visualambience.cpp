...
void VAmbienceApplySky(bool bDisable = false)
{
	...
	GetConVarString(hSkyname, VAmbienceDefaultSky, sizeof(VAmbienceDefaultSky));
	...
	if(bDisable)
	{
		if(strlen(VAmbienceDefaultSky))
		{
			...
			SetConVarString(hSkyname, VAmbienceDefaultSky, true);
		}
		return;
	}
...
void VAmbienceApplyFog(/*void*/)
{
	...
	if(!GetConVarBool(gCvarList[CVAR_VEFFECTS_FOG]))
	{
		return;
	}
	...
	if(!IsValidEdict(iFogControllerIndex))
	{
		iFogControllerIndex = CreateEntityByName("env_fog_controller");
		if(iFogControllerIndex) DispatchSpawn(iFogControllerIndex);
	}

...
#define LOG_DEBUG                   (1 << 3)    /** Log debug messages, ifany. Usually only developers enable this log flag. */
...
/**
 * List of modules that write log events. Add new modules ifneeded (in
 * alphabetical order).
 * 
 ...
 * @return              	The matcing module type or LogModules_Invalid iffailed.
 **/
LogModules LogGetModule(char[] sModuleName)
{
	...
	else if(StrEqual(sModuleName, "menus", false))
	{
		return LogModule_Menus;
	}
	else if(StrEqual(sModuleName, "antistick", false))
	{
	...
	else if(StrEqual(sModuleName, "humanclasses", false))
	{
...
/**
 * Check ifthe specified log flag is set.
 *
 * @param eventType			The log flag to check.
 * @return              	True ifset, false otherwise.
 **/
...
/**
 * Check ifthe specified module is enabled in the LOG_MODULE filter cache.
 *
...
{    
	...
	if((logType != LogType_Fatal && logType != LogType_Error) || (logType == LogType_Error && !GetConVarBool(gCvarList[CVAR_LOG_ERROR_OVERRIDE])))
	{
		...
		{
			// Check ifthe specified module is enabled
	...
	static char sModule[NORMAL_LINE_LENGTH];
	...
	// Check ifprinting log events to public chat is enabled
 ...
 * @return          		True ifadded, false otherwise.
 **/
bool LogModuleFilterAdd(LogModules iModule)
{
    static char sModuleName[NORMAL_LINE_LENGTH];
    ...
    // Check ifthe module isn't already is listed

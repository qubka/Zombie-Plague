...
enum ParamParseResult
{
    ...
    String:Param_Value[PARAM_VALUE_MAXLEN]  /** Value. Only used ifa key. */
}
...
stock int ParamParseString(iBuffer[][ParamParseResult], char[] sParamString, int iMaxLen, int &iKeys = 0)
{
	...
	// Check ifstring is empty
	if(!iLen)
	{
		return PARAM_ERROR_EMPTY;
	}

	// Check ifthere's space left in the destination buffer
	...
	while(iMode != ParamMode_Finish)
	{
		...
		switch (iMode)
		{
			...
			case ParamMode_Key :
			{
				...
				// Check ifstring is empty, then stop
				if(!strlen(sValue))
				{
 ...
 * @return					Index of the key iffound, -1 otherwise.
 **/

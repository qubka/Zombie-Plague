...
/*
    ...
    * The return value is true ifthe file was successfully loaded, false ifthe
      config file couldn't be loaded.  (Invalid data, missing quotes, brackets, etc)
    
    -Next validate the config so far, stopping ifno data was found or ifConfigLoadConfig
     returned false.
...
enum ConfigData
{
    bool:Data_Loaded,                    	/** True ifconfig is loaded, false ifnot. */
...
void ConfigLoad(/*void*/)
{
	...
	static char sPath[BIG_LINE_LENGTH];
	...
	if(FileExists(sPath))
	{
		...
		// Log action
		LogEvent(false, LogType_Normal, LOG_CORE_EVENTS, LogModule_Config, "Map Configs", "Executed main config file: %s", sPath);
	}
...
/**
 * Returns ifa config was successfully loaded.
 * 
 * @param iConfig    		Config file to check load status of.
 * @return           		True ifconfig is loaded, false otherwise.
 **/
 ...
 * @return               	True iffile was loaded successfuly, false otherwise.
 **/
 ...
 * @return           		True ifthe config is loaded, false ifnot.
 **/
 ...
 * @return              	True ifthe change was made successfully, false otherwise. 
 **/
stock bool ConfigKeyvalueTreeSetting(ConfigFile iConfig, ConfigKvAction iAction = KvAction_Create, const char[][] sKeys, int keysMax, const char[] sSetting = "", char[] sValue = "", int iMaxLen = 0)
{
    ...
    // i = keys index.
    // Traverse into the keygroup, stop ifit fails
    for (int i = 0; i < keysMax; v++)
    {
        ...
        // Try to jump to next level in the transversal stack, create key ifspecified
 ...
 *
 * @return          		Config file entry, ConfigInvalid is returned ifalias was not found.
 **/
 ...
 * @return     	   			True ifthe file exists, false ifnot.
 **/
 ...
 * @return          		 True ifstring is "yes", false otherwise.
 **/
 ...
 * @param sDefaultValue  	(Optional) Value to return ifsetting is missing.
 **/

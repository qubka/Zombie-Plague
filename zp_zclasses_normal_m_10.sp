...
{
    // Classes
    gZombie = ZP_GetZombieClassNameID("normalm10");
    if(gZombie == -1) SetFailState("[ZP] Custom zombie class ID from name : \"normalm10\" wasn't find");
...
public Action ZP_OnClientSkillUsed(int clientIndex)
{
    ...
    if(ZP_IsPlayerZombie(clientIndex) && ZP_GetClientZombieClass(clientIndex) == gZombie)
    {
        ...
        if(entityIndex != INVALID_ENT_REFERENCE)
        {
            ...
            // Create an effect
            FakeCreateParticle(entityIndex, vPosition, _, "gamma_blue", ZP_GetZombieClassSkillDuration(gZombie));
            
            // Put fire on it
            IgniteEntity(entityIndex, ZP_GetZombieClassSkillDuration(gZombie));
            ...
            // Create remove timer
            CreateTimer(ZP_GetZombieClassSkillDuration(gZombie), BlastExploadHook, EntIndexToEntRef(entityIndex), TIMER_FLAG_NO_MAPCHANGE);
        }
    }
        ...
        for(int i = 1; i <= MaxClients; i++)
        {
            ...
            if(IsPlayerExist(i) && ((ZP_IsPlayerHuman(i) && !ZP_IsPlayerSurvivor(i)) || (ZP_IsPlayerSurvivor(i) && ZOMBIE_CLASS_SKILL_EXP_SURVIVOR)))
            {
                ...
                if(flDistance <= ZOMBIE_CLASS_SKILL_EXP_RADIUS)
                {
                    ...
                    // Create a fade
                    FakeCreateFadeScreen(i, ZOMBIE_CLASS_SKILL_DURATION_F, ZOMBIE_CLASS_SKILL_TIME_F, 0x0001, ZOMBIE_CLASS_SKILL_COLOR_F);
                    
                    // Create a shake
                    FakeCreateShakeScreen(i, ZOMBIE_CLASS_SKILL_SHAKE_AMP, ZOMBIE_CLASS_SKILL_SHAKE_FREQUENCY, ZOMBIE_CLASS_SKILL_SHAKE_DURATION);

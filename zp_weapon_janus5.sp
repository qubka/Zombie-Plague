...
void Weapon_OnPrimaryAttack(int client, int weapon, int iClip, int iAmmo, int iCounter, int iStateMode, float flCurrentTime)
{
	...
	float vKickback[] = { /*upBase = */0.35, /* lateralBase = */0.4, /* upMod = */0.15, /* lateralMod = */0.05, /* upMax = */1.5, /* lateralMax = */1.5, /* directionChange = */5.0 };
...
public void ZP_OnWeaponBullet(int client, float vBullet[3], int weapon, int weaponID)
{
	if (weaponID == gWeapon)
	{
		ZP_CreateWeaponTracer(client, weapon, "1", "muzzle_flash", "weapon_tracers_assrifle", vBullet, ZP_GetWeaponShoot(gWeapon));
	}
}

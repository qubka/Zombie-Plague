...
{
    // Classes
    gZombie = ZP_GetZombieClassNameID("normalm02");
    if(gZombie == -1) SetFailState("[ZP] Custom zombie class ID from name : \"normalm02\" wasn't find");
...
public Action ZP_OnClientSkillUsed(int clientIndex)
{
    ...
    if(ZP_IsPlayerZombie(clientIndex) && ZP_GetClientZombieClass(clientIndex) == gZombie)
    {
        ...
        if(entityIndex != INVALID_ENT_REFERENCE)
        {
            ...
            // Create remove timer
            CreateTimer(ZP_GetZombieClassSkillDuration(gZombie), CoffinExploadHook, EntIndexToEntRef(entityIndex), TIMER_FLAG_NO_MAPCHANGE);
        }
    }

...
// Cvars
ConVar hCvarCartblueMultiplier;
...
{
	hCvarCartblueMultiplier = CreateConVar("zp_weapon_cartblue_active_multiplier", "2.0", "Multiplier on the active state", 0, true, 0.0);
...
void Weapon_OnPrimaryAttack(int client, int weapon, int iClip, int iAmmo, int iStateMode, float flCurrentTime)
{
	...
	if (iStateMode)
	{
		...
		SetEntPropFloat(weapon, Prop_Send, "m_fLastShotTime", flCurrentTime + (ZP_GetWeaponShoot(gWeapon) * hCvarCartblueMultiplier.FloatValue));       
	...
	else
	{
		...
		UTIL_CreateParticle(view, _, _, "3", sName, 0.1);
	}
	...
	float vKickback[] = { /*upBase = */0.15, /* lateralBase = */0.35, /* upMod = */0.1, /* lateralMod = */0.025, /* upMax = */1.5, /* lateralMax = */2.5, /* directionChange = */5.0 };
...
void Weapon_OnSecondaryAttack(int client, int weapon, int iClip, int iAmmo, int iStateMode, float flCurrentTime)
{
	...
	ZP_SetPlayerAnimation(client, !iStateMode ? PLAYERANIMEVENT_SILENCER_ATTACH : PLAYERANIMEVENT_SILENCER_DETACH);
...
public void ZP_OnClientValidateDamage(int client, int &attacker, int &inflictor, float &flDamage, int &iBits, int &weapon)
{
	if (iBits & DMG_NEVERGIB)
	{
		if (IsValidEdict(weapon))
		{
			...
			{
				if (GetEntProp(weapon, Prop_Data, "m_iSecondaryAmmoCount")) flDamage *= hCvarCartblueMultiplier.FloatValue;
			}
		}
	}
}

...
{
    // Classes
    gZombie = ZP_GetZombieClassNameID("normalm08");
    if(gZombie == -1) SetFailState("[ZP] Custom zombie class ID from name : \"normalm08\" wasn't find");
        ...
        for(int i = 1; i <= MaxClients; i++)
        {
            ...
            if(IsPlayerExist(i) && ((ZP_IsPlayerHuman(i) && !ZP_IsPlayerSurvivor(i)) || (ZP_IsPlayerSurvivor(i) && ZOMBIE_CLASS_SKILL_SURVIVOR)))
            {
                ...
                {            
                    ...
                    // Create an fade
                    FakeCreateFadeScreen(i, ZOMBIE_CLASS_SKILL_DURATION_F, ZOMBIE_CLASS_SKILL_TIME_F, 0x0001, vColor);

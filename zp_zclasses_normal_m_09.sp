...
{
    // Classes
    gZombie = ZP_GetZombieClassNameID("normalm09");
    if(gZombie == -1) SetFailState("[ZP] Custom zombie class ID from name : \"normalm09\" wasn't find");
...
public Action ZP_OnClientSkillUsed(int clientIndex)
{
    ...
    if(ZP_IsPlayerZombie(clientIndex) && ZP_GetClientZombieClass(clientIndex) == gZombie)
    {
        ...
        // Create an fade
        FakeCreateFadeScreen(clientIndex, ZOMBIE_CLASS_SKILL_DURATION_F, ZOMBIE_CLASS_SKILL_TIME_F, 0x0001, ZOMBIE_CLASS_SKILL_COLOR_F);  
        ...
        // Create an effect
        FakeCreateParticle(clientIndex, vEntPosition, _, "tornado", ZP_GetZombieClassSkillDuration(gZombie));
            ...
            if(IsPlayerExist(i) && ((ZP_IsPlayerZombie(i) && !ZP_IsPlayerNemesis(i)) || (ZP_IsPlayerNemesis(i) && ZOMBIE_CLASS_SKILL_NEMESIS)))
            {
                ...
                if(flDistance <= ZOMBIE_CLASS_SKILL_RADIUS)
                {
                    ...
                    if(GetClientHealth(i) < iHealth)
                    {
                        ...
                        // Create an effect
                        FakeCreateParticle(i, vVictimPosition, _, "heal_ss", ZP_GetZombieClassSkillDuration(gZombie));
